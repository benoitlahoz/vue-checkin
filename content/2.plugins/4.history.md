...
The History Plugin tracks a complete history of all operations performed on the desk with timestamps. It's perfect for audit trails, undo/redo functionality, debugging, and analytics.
...
When the limit is reached, the oldest entries are automatically removed as new ones are added.
...
<template>
  <div class="history-timeline">
    ...
    <button @click="desk.clearHistory()">Clear History</button>
  </div>
</template>
```
...
```vue
<script setup lang="ts">
const { desk } = createDesk('editable-items', {
...
const canRedo = computed(() => {
  const history = desk.getHistory();
  return currentIndex.value < history.length - 1;
});
...
const undo = () => {
  ...
  currentIndex.value--;
  const entry = desk.getHistory()[currentIndex.value];
  
  // Restore previous state
  if (entry.action === 'check-in' || entry.action === 'update') {
    desk.update(entry.id, entry.data);
  } else if (entry.action === 'check-out') {
    // Re-check in the item
    const previousEntry = desk.getHistoryByAction('check-in')
      .reverse()
      .find(e => e.id === entry.id);
    if (previousEntry?.data) {
      desk.checkIn(entry.id, previousEntry.data);
    }
  }
};
...
const redo = () => {
  ...
  if (entry.action === 'check-in') {
    desk.checkIn(entry.id, entry.data);
  } else if (entry.action === 'update') {
    desk.update(entry.id, entry.data);
  } else if (entry.action === 'check-out') {
    desk.checkOut(entry.id);
  }
};
</script>
