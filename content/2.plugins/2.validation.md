...
desk.checkIn('user-1', { name: 'John' });

// ❌ This will fail - invalid email
desk.checkIn('user-2', { name: 'Jane', email: 'invalid' });

// ❌ This will fail - custom validation
desk.checkIn('user-3', { name: 'Bob', email: 'bob@example.com', age: 16 });
...
const submitForm = () => {
  ...
  // Try to check in
  const success = desk.checkIn('form-data', formData);
  
  if (!success) {
    // Get the error message
    const error = desk.getLastValidationError();
    if (error) {
      alert(error.message);
    }
  } else {
...
**Prevention, not just notification**: The Validation Plugin returns `false` from `onBeforeCheckIn` to **prevent** invalid data from being checked in. The check-in operation will fail if validation fails.
::
...
See the [Form Example](/examples/form-example) for a complete working implementation.
